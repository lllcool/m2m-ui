<template>
  <div class="app-container">
    ...
    <!-- 新建表单 -->
    <member-add ref="memberAdd" @created="doQueryList({ page: 1 })"/>
    <!-- 编辑表单 -->
    <member-edit ref="memberEdit" @updated="doQueryList({})"/>
    <!-- 查看表单 -->
    <member-show ref="memberShow"/>
    <!-- 查看表单 -->
    <member-import ref="memberImport" @imported="doQueryList({ page: 1 })"/>
  </div>
</template>
...
import memberImport from './import'
...
export default {
  ...
  methods: {
    ...
    doQueryList({ page, limit }) {
      ...
      return memberApi.fetchList(this.query)
        .then(data => {
          this.list = data.list
    ...
    handleDeleteSingle(row) {
      return this.$common.confirm('是否确认删除')
        .then(() => memberApi.deleteById(row.id))
        .then(() => {
    ...
    handleDeleteBatch() {
      if (this.selectItems.length <= 0) {
        this.$common.showMsg('warning', '请选择Member')
        return
      }
      return this.$common.confirm('是否确认删除')
        .then(() => memberApi.deleteBatch(this.selectItems.map(row => row.id)))
        .then(() => {
          this.$common.showMsg('success', '删除成功')
          return this.doQueryList({ page: 1 })
        })
     ...
     */
    handleCreate() {
      this.$refs.memberAdd.handleCreate()
    },
    /**
     * 打开查看表单
     */
    handleShow(row) {
      this.$refs.memberShow.handleShow(row.id)
    },
    /**
     * 打开编辑表单
     */
    handleUpdate(row) {
      this.$refs.memberEdit.handleUpdate(row.id)
    },
    /**
     * 导出excel
     */
    handleExport() {
      return this.$common.confirm('是否确认导出')
        .then(() => memberApi.exportExcel(this.query))
    },
    /**
     * 打开导入表单
     */
    handleImport() {
      this.$refs.memberImport.show()
    }
  }
}
</script>
